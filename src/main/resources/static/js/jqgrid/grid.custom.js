...
$.jgrid.extend({
	...
	filterToolbar : function(p){
		...
		return this.each(function(){
			...
			//PathSolutions customization: adding Toggle/Clear in the pager to show/hide toolbar + clear search filter
			var pagerId = '#'+this.id+'_pager';
			var mygrid = $(this).jqGrid({pager : pagerId});
			$(this).jqGrid('navButtonAdd',pagerId,{caption:$t.p.toggletext,title:$t.p.toggletitle, buttonicon :'ui-icon-pin-s', onClickButton:function(){ mygrid[0].toggleToolbar() } });
			$(this).jqGrid('navButtonAdd',pagerId,{caption:$t.p.cleartext,title:$t.p.cleartitle,buttonicon :'ui-icon-refresh', onClickButton:function(){ mygrid[0].clearToolbar() } });
			//end
			...
			var triggerToolbar = function() {
				...
				$.each($t.p.colModel,function(i,n){
					...
					if(v) {
						sdata[nm] = v;
						/**
						 * [PathSolutions-MarwanMaddah]: added to catch that the column that is a number to do the regEx on it inside GridBaseAction on filter build.
						 * BUG#542174 On entering valid process code data with 2 digits decimal point, search operation is not working properly in Process Code field
			...
			var clearToolbar = function(trigger){
				...
				//PathSolution Denisk, Added in order to clear the filter inside Advanced Search when Clear is clicked in Grid footer
				$("#fbox_"+$t.p.id+"_reset").trigger("click"); 
			...
			$.each($t.p.colModel,function(i,n){
				...
				if(this.search){
					...
					case 'text':
						...
						//PathSolutions addition to restrict adding characters for columns of type number 
						$("input",thd).keydown(function(event)
				        {
							 switch (event.which)
					         {
							 	case 9: //PathSolutions: tab should invoke scroll to postion
							 		if($.browser.msie && $t.p.direction=== "rtl")
						 			{
								 		var leftPosInRtl = $(this).offset().left + $(this).outerWidth();
							 			var bdivLeftInRtl = $($t.grid.bDiv).offset().left + $($t.grid.bDiv).outerWidth();
							 			$t.grid.bDiv.scrollLeft += bdivLeftInRtl -leftPosInRtl ;  
						 			}
							 		else
						 			{
								 		$t.grid.bDiv.scrollLeft += $(this).offset().left - $($t.grid.bDiv).offset().left; 
						 			}
							 		break;
							 	//down arrow key
								case 40: //select first row while down arrow pressed from filter columns
									var ids = $($t).jqGrid("getDataIDs");
									if(ids && ids.length > 0)
									{
										$($t).find("tr[id='"+ids[0]+"']").focus();
										$($t).find("tr[id='"+ids[0]+"']").click();
									}
									break;
							 }
							if(cm.colType == "number")
							{
					            switch (event.which)
					            {
						...
						//added keyup to check on number column type if the value is a number to keep it otherwise remove it
						if(cm.colType == "number")
						{
							$("input",thd).keyup(function(event)
				        	{
	...
	setGroupHeaders : function ( o ) {
		...
		return this.each(function(){
			...
			inColumnHeader = function (text, columnHeaders) {
				...
				for (; i < length; i++) {
					//PathSolutions. add a condition on undefined columnHeaders to avoid the case [object,]
					if ( columnHeaders[i] != undefined && columnHeaders[i].startColumnName === text) {
						return i;
					}
				}
				return -1;
			...
			for (i = 0; i < cml; i++) {
				...
				if (iCol >= 0) {
					...
					if(cVisibleColumns > 0) {
						...
						//PathSolutions double header Coloring management
						$colHeader.addClass("path-double-header");
					}
					...
					if (skip === 0) {
						if (o.useColSpanStyle) {
							...
							if($th.parent().parent().children("tr").length>1)
							{
								$th.attr("rowspan", $th.parent().parent().children("tr").length);
							}else
							{
								$th.attr("rowspan","2");
							}
