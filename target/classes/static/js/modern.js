 ...
 * @returns {Boolean}
 */
function validateGenericAllRequiredByFormId(formId)
{
	var flag=true;
	$(formId+" .validate-required-error").remove();
	$(formId+" .validate-required").each(function(i)
	{
		if($.trim( $(this).val())=="")
		{
...
{
	var flag = true;
	if( !validateGenericAllRequiredByFormId(formId))
	{
		if(flag)
		{
			flag=false;
...
{
	var flag = true;
	if( !validateGenericAllRequired(formId))
	{
		if(flag)
		{
			flag=false;
...
function clearFields(formId)
{
	$.each($('input'),function(i,val){
		...
		if(typeof $(this).attr("type") != "undefiend")
		{
	...
	$("#"+formId+" input:text,#"+formId+" input:file ,#"+formId+" select,#"+formId+" textarea,#"+formId+" hidden").each(function(){
		$("#"+$(this).attr('id')).val("");
	});
	...
	$("#"+formId+" :radio").each(function () {
	 ...
	 $("#"+formId+" table").each(function(){
		 $("#"+$(this).attr('id')).children('tr:not(:first)' ).remove();
     });
