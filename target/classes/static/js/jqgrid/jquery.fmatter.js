...
//			console.log(">>>>>>>>>>>>>>>> DateFormat date "+date)
			...
			if( !isNaN( date - 0 ) && String(format).toLowerCase() == "u") {
				...
				//PathSolutions: in case getHours is 23 and the day is different, means daylight adjustment needed
				if(timestamp.getHours() == 23 && timestamp.getDate() != ts.d) 
					timestamp.setHours(timestamp.getHours() + 2);//just to shift the days when its daylight saving date
			}
	...
	$.fn.fmatter.checkbox =function(cval, opts) {
		...
		var bchk = cval.search(/(false|0|no|off|n)/i)<0 ? " checked='checked' " : ""; //PathSolutions: added |n to cover the case of y:n
		/**
		 * [PathSolutions - MarwanMaddah]: added id to the input and onClick function
		 * to call the function that will be defined on the editOptions
		 * to call the dependency in case the grid is editable and only the checkBox is enable based on the formatoptions column attribute 
		 */
		var onClickStr = "";
		if(opts.colModel.editoptions!=null 
		   && opts.colModel.editoptions!="undefined" 
		   && opts.colModel.editoptions.dataEvents!=null 
		   && opts.colModel.editoptions.dataEvents!="undefined"
		   && opts.colModel.editoptions.dataEvents[0].fn !=null && opts.colModel.editoptions.dataEvents[0].fn!="undefined")
		{
		   var funcName = opts.colModel.editoptions.dataEvents[0].fn;
		   onClickStr = "onclick=\"$('#"+opts.gid+"').jqGrid('setSelection','"+opts.rowId+"'); eval("+funcName+").call();\"";
		}
		return "<input class='path-dummy-cls' id='"+opts.rowId+"_"+opts.colModel.name+"' "+onClickStr+" type=\"checkbox\" " + bchk  + " value=\""+ cval+"\" offval=\"no\" "+ds+ "/>"; 	
	};
	...
	$.fn.fmatter.date = function (cellval, opts, rwd, act) {
		...
		//PathSolutions: added validation on date apply formatting only when cellval is string date ISO format or cellval is date object  
		var validDateVal = (typeof cellval!= "undefined" && (typeof cellval == "object" || parseISODateStr(cellval) != null )) ? true : false;
		if(!$.fmatter.isUndefined(opts.colModel.formatoptions)) {
			op = $.extend({},op,opts.colModel.formatoptions);
		}
		if(!op.reformatAfterEdit && act=='edit'){
			return $.fn.fmatter.defaultFormat(cellval, opts);
		} else if(!$.fmatter.isEmpty(cellval) && validDateVal ) {
	...
	$.fn.fmatter.select = function (cellval,opts, rwd, act) {
		...
		if (oSelect) {
			...
			{
				if( $.isFunction(oSelect))
				{
					var res;
					if (typeof opts.colModel.combos != "undefined" && typeof opts.colModel.combos[opts.colModel.name] != "undefined" ) 
					{ 
						var tmp = JSON.stringify(opts.colModel.combos[opts.colModel.name]);
						res = tmp.substring(1,tmp.length -1) //oSelect.call();
					} 
					else 
				    {
					  opts.colModel.combos = $.extend({},opts.colModel.combos);
					  res = oSelect.call();
					  opts.colModel.combos[opts.colModel.name] = res;
					}
			...
			else if($.fmatter.isString(oSelect)) 
			{
	...
	$.unformat.select = function (cellval,options,pos,cnt) {
		...
		if(op.value){
			...
			//*******************PathSolutions checking if combo is loaded through javascript function ************************/
			if($.isFunction(op.value) )
			{
				//call the function and return the real value in form of key:value, key:value...
				if(options.colModel.combos && options.colModel.combos[options.colModel.name])
				{
					var tmp = JSON.stringify(options.colModel.combos[options.colModel.name])
					oSelect = tmp.substring(1,tmp.length -1) //oSelect.call();
				}
				else
				{
					oSelect = oSelect.call();
				}
			}
			else
				oSelect = op.value;
