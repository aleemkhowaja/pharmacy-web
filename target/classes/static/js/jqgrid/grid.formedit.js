...
$.jgrid.extend({
	searchGrid : function (p) {
		...
		return this.each(function() {
			...
			function showFilter() {
				...
				//[pathsolutions] fix #494894 . when the grid is loaded inside an iframe, the advanced search dialog comes behind the grid, the z-index should be set to 1 to fix the display
				//[path Solutions] #BUG 530578 caused by 494894. (the search is not operating anymore if it is used from normal screen and not an iFrame
				//added checking if z-index is auto then set it to 1)
				if($("#"+IDs.themodal).css('z-index') == "auto")
				{
					$("#"+IDs.themodal).css('z-index','1');
				}
			}
			if ( $("#"+IDs.themodal).html() !== null ) {
				...
				/**
				 * [PathSolutions]:Used on advance search \ Find Click will enter here
				 */
				$("#"+fid+"_search").bind('click', function(){
					...
					if(p.closeAfterSearch) {
						//in case find btn is clicked we need to hide all opened datepickers
			...
			function createData(rowid,obj,tb,maxcols){
				...
				$(obj.p.colModel).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && this.editable===true && nm !== 'rn') {
						...
						var opt = $.extend({},{colModel:this} ,this.editoptions || {} ,{id:nm,name:nm}),
	...
	delGridRow : function(rowids,p) {
		...
		return this.each(function(){
			...
			if ( $("#"+IDs.themodal).html() !== null ) {
				...
				// PathSolution to make the delete Row grid Popup confirmation positioned properly
				if(!p.deltop && !p.delleft) 
				{
                      if (typeof window.innerWidth != 'undefined') {
                            p.delleft = window.innerWidth;
                            p.deltop = window.innerHeight;
                      } else if (typeof document.documentElement != 'undefined' && typeof document.documentElement.clientWidth != 'undefined' && document.documentElement.clientWidth !== 0) {
                            p.delleft = document.documentElement.clientWidth;
                            p.deltop = document.documentElement.clientHeight;
                      } else {
                            p.delleft=1024;
                            p.deltop=768;
                      }
                      p.delleft = p.delleft/2 - parseInt(p.width,10)/2;
                      p.deltop = p.deltop/2-25;
                }
                var theBox = {gbox:"#gbox_"+gID,jqModal:true,drag:true,resize:true,top:p.deltop,left:p.delleft,caption:rp_ge[$t.p.id].caption,height: p.height};
				$.jgrid.createModal(IDs,tbl,theBox,"#gview_"+$t.p.id,$("#gview_"+$t.p.id)[0],true);
				...
				$("#dData","#"+dtbl+"_2").click(function(e){
					...
					if(ret[0] && !rp_ge[$t.p.id].processing) {
						...
						idname = opers.id;
						//PathSolutions: changed assigning of same variable to an array, used postDataArr instead
						//postdata = postdata.split(",");		
						var postdataArr = postdata.split(",");	 
						for( var pk in postdataArr) {
							if(postdataArr.hasOwnProperty(pk)) {
								postdataArr[pk] = $.jgrid.stripPref($t.p.idPrefix, postdataArr[pk]);
							}
						}
						postd[idname] = postdataArr.join();
	...
	navGrid : function (elem, o, pEdit,pAdd,pDel,pSearch, pView) {
		...
		return this.each(function() {
			if(this.nav) {return;}
			//PathSolution to make a dialog specific for every Grid
			var alertIDs = {themodal:'alertmod'+this.p.id,modalhead:'alerthd'+this.p.id,modalcontent:'alertcnt'+this.p.id},
			$t = this, twd, tdw;
				...
				if (o.add) {
					...
					.click(function(){
						if (!$(this).hasClass('ui-state-disabled')) {
							//PathSolutions should call saverow to get the changes for row in case u want to send parameters
							if($t.p.savedRow.length > 0)
								{
									$($t).jqGrid("saveRow",$t.p.savedRow[0].id ,false, 'clientArray');
								}
							//end saverow
							
							//PathSolutions customization: call the add function if specified in grid tld attribute addfunc
							if (typeof o.addfunc == 'string') {
								if(o.addfunc.indexOf("(") >-1)	//in case parameters needed to be sent to addfunc
									jQuery.globalEval( o.addfunc);
								else
									jQuery.globalEval( o.addfunc + "()" );
							} //*******************************/
							else 
...
//									jQuery.globalEval( o.editfunc + "("+sr+")" );
									eval( o.editfunc + "(sr)" );
								} //*******************************/
				...
				if (o.view) {
					...
					.click(function(){
						if (!$(this).hasClass('ui-state-disabled')) {
							var sr = $t.p.selrow;
							if (sr) {
							//PATHSolutions customization: call the view function if specified in grid tld attribute viewfunc 
								if (typeof o.viewfunc == 'string') {
								jQuery.globalEval( o.viewfunc + "("+sr+")" );
								}
								else
				...
				if (o.del) {
					...
					.click(function(){
						if (!$(this).hasClass('ui-state-disabled')) {
							...
							if(dr){
								//PathSolutions customization: call the delete function if specified in grid tld attribute delfunc
								if (typeof o.delfunc == 'string') {
									if(o.delfunc.indexOf("(") >-1)	//in case parameters needed to be sent to delfunc
										jQuery.globalEval( o.delfunc);
									else
										eval( o.delfunc + "(dr)" );
								}//*********************/ 
								else  
				...
				//
				// PathSolutions : [KHALED] -- Add button to export the grid into excel file
				//
				if(o.exportExcel) {
					tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
					$(tbd).append("<div class='ui-pg-div'><span class='ui-icon ui-icon-squaresmall-close'></span></div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
					.attr({"title":o.exportText  || "",id: "exportToExcel_"+elemids})
					.click(function(){
						// get the last page
						var lastPage = $($t).jqGrid('getGridParam', 'lastpage');
						
						//
						// Check if there is a need to ask the user to export all
						// pages or just proceed to download
						//
						var proceed = (lastPage && lastPage > 1 ) ? false : true;
						
						var excelInfo = {};
						excelInfo.gridColModel = $t.initOpts.colModel;
						excelInfo.gridData = $($t).jqGrid('getRowData') || {};
						if($t.p.groupHeader) {
							excelInfo.gridGroupHeaders = $t.p.groupHeader.groupHeaders || {};
						}
						if($t.p.userData) {
							excelInfo.gridFooter = $t.p.userData || {};
						}
						// Create dynamic form to store the values of the excel info
						var gridId = $($t).attr('id');
						$('<form />', { id: 'formExportExcelTemp_' + gridId }).appendTo('body');
						$('<input>').attr({
						    type: 'hidden',
						    id: 'tempId_ExcelExport_hid_' + gridId,
						    name: 'exportGridStr',
						    value: encodeURI(JSON.stringify(excelInfo))
						}).appendTo('#formExportExcelTemp_' + gridId);
						
						$('<input>').attr({
						    type: 'hidden',
						    id: 'tempIdNames_ExcelExport_hid_' + gridId,
						    name: 'gridColNamesStr',
						    value: encodeURI(JSON.stringify($t.initOpts.colNames))
						}).appendTo('#formExportExcelTemp_' + gridId);
						
						// last page is 1
						if(proceed) {
							_showProgressBar(true)
							$.fileDownload(jQuery.contextPath + '/path/gridBaseAction_exportGridToExcel', {
								    successCallback: function (url) {
										_showProgressBar(false);
										$('#formExportExcelTemp_' + gridId).remove();
								    },
								    ...
								    httpMethod: "POST",
							    	data: $('#formExportExcelTemp_' + gridId).serialize()
							});
						} else {
							...
							var targetUrl, targetUrlWithParam, inUrlParam;
							
							// Target Method
							var method = 'exportGridToExcelAllPages';
							
							// Get the method name
							var methodName;
							if(url && url.indexOf("_") > -1) {
								if(url.indexOf(".action?") > -1){
									var turl = url.substring(0, url.lastIndexOf(".action?"));
									methodName = turl.substring(turl.lastIndexOf("_") + 1);
									targetUrl = turl.substring(0, turl.lastIndexOf(methodName)) + method + '?';
									inUrlParam = url.substring(url.lastIndexOf(".action?") + 8);
								} 
								else if(url.indexOf(".action") > -1){
									var turl = url.substring(0, url.lastIndexOf(".action"));
									methodName = turl.substring(turl.lastIndexOf("_") + 1);
									targetUrl = turl.substring(0, turl.lastIndexOf(methodName)) + method + '?';
									inUrlParam = url.substring(url.lastIndexOf(".action") + 7);
									
								} else if(url.indexOf("?") > -1){
									var turl = url.substring(0, url.lastIndexOf("?"));
									methodName = turl.substring(turl.lastIndexOf("_") + 1);
									targetUrl = turl.substring(0, turl.lastIndexOf(methodName)) + method + '?';
									inUrlParam = url.substring(url.lastIndexOf("?") + 1);
								} else {
									methodName = url.substring(url.lastIndexOf("_") + 1);
									targetUrl = url.substring(0, url.lastIndexOf(methodName)) + method + '?';
									inUrlParam = "";
								}
							}
							targetUrlWithParam = targetUrl;

							var postData = $($t).jqGrid('getGridParam', 'postData');
							$.each(postData, function(key, value) {
							  	targetUrlWithParam += "&"+key+"="+encodeURIComponent(value);
							});
							targetUrlWithParam += "&" +  inUrlParam;

							$('<input>').attr({
						    type: 'hidden',
						    id: 'tempURL_ExcelExport_hid_'+ gridId,
						    name: 'gridURLStr',
						    value: encodeURI(JSON.stringify(targetUrlWithParam))
						}).appendTo('#formExportExcelTemp_' + gridId);
						
						$('<input>').attr({
						    type: 'hidden',
						    id: 'tempMethodName_ExcelExport_hid_' + gridId,
						    name: 'gridMethodStr',
						    value: methodName
						}).appendTo('#formExportExcelTemp_' + gridId);
							
						var currentPageUrl = jQuery.contextPath + '/path/gridBaseAction_exportGridToExcel';
						
						var $dialogDiv = $('<div id="temp_dialog_export_excel_' +  gridId + '"><fieldset>' +
						'<input type="radio" name="radio-group-export_' +  gridId + '" id="radio-all-pages" value="1" checked/>' +
						'<label for="radio-all-pages">' + o.exportAllPagesLbl + '</label>' +
						'</br><input type="radio" name="radio-group-export_' +  gridId + '" id="radio-current-page" value="0"/>' +
						'<label for="radio-all-pages">' + o.exportCurrentPageLbl + '</label>' +
						'</fieldset></div>').appendTo('body');
						var dialog = $('#temp_dialog_export_excel_' +  gridId ).dialog({ modal: true });
						...
						var dialogOptions = {
								...
								'width' : returnMaxWidth(220),
								buttons : [
										{
											text : o.exportOkBtn,
											click : function() {
												var value = $('input[name=radio-group-export_' +  gridId + ']:checked').val();
												var isAll = (value && value == "1") ? true : false ;
												var goToUrl = isAll ? targetUrl : currentPageUrl;
												_showProgressBar(true);
												$.fileDownload( goToUrl, {
													    successCallback: function (url) {
															_showProgressBar(false);
															$('#formExportExcelTemp_' + gridId).remove();
													    },
												...
												$('#temp_dialog_export_excel_' +  gridId ).dialog('destroy');
												$('#temp_dialog_export_excel_' +  gridId ).remove();
											}
										}
									]
						};
						$('#temp_dialog_export_excel_' +  gridId ).dialog(dialogOptions);
						$('#temp_dialog_export_excel_' +  gridId ).dialog('open');
					}
					...
					tbd = null;
					// Add separator to the excel icon for better visuality 
				...
				if (o.search) {
					...
					.click(function(){
						if (!$(this).hasClass('ui-state-disabled')) {
							//in case the filter dialog is closed we need to close all opened datepickers
							pSearch.onClose = function () {$.datepicker._hideDatepicker();};
				...
				/**
				 * [pathSolution-MarwanMaddah]: in case the grid is sortable
				 * Reset preferences icon will be added to the grid navigator to reset the preferences on click  
				 */
				if($t.p.sortable)
				{
					tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
					$(tbd).append("<div class='ui-pg-div'><span class='ui-icon ui-icon-newwin'></span></div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
					.attr({"title":o.resetUserPreferences  || "",id: "userPref_"+elemids})
					.click(function(){
						/**
						 * function exists in commonFuncExtension.js
						 * @memberOf {TypeName} 
						 */
						var newOpts = {};
						jQuery.extend(newOpts,$t.p);
						newOpts.colModel = $t.initOpts.colModel;
						newOpts.colNames = $t.initOpts.colNames;
						newOpts.columnsOrder = null;
						newOpts.remapColumns = [];
						resetUserGridPreferences(newOpts.id);
					}).hover(
						function () {
							if (!$(this).hasClass('ui-state-disabled')) {
								$(this).addClass("ui-state-hover");
					...
					if($t.p.gridColSortRight)
					{
						tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
						$(tbd).append("<div class='ui-pg-div'><span class='ui-icon ui-icon-person'></span></div>");
						$("tr",navtbl).append(tbd);
						$(tbd,navtbl)
						.attr({"title":o.setUserDefaultPreferences  || "",id: "userDefPref_"+elemids})
						.click(function(){
							/**
							 * function exists in commonFuncExtension.js
							 * @memberOf {TypeName} 
							...
							var newOpts = {};
							jQuery.extend(newOpts,$t.p);
							setUserDefaultGridPreferences(newOpts);
						}).hover(
							function () {
								if (!$(this).hasClass('ui-state-disabled')) {
									$(this).addClass("ui-state-hover");
				...
				if($t.p._nvtd) {
					if(twd > $t.p._nvtd[0] ) {
//						$(pgid+"_"+o.position,pgid).width(twd); //pathsolutions: removed as per latest jqgrid
						$t.p._nvtd[0] = twd;
					}
					else //PathSolutions: fixing width to have the pager at center
					{
						$(pgid+"_"+o.position,pgid).width( $t.p._nvtd[0]);
					}
					$t.p._nvtd[1] = twd;
				}
		...
		return this.each(function() {
			...
			if (findnav) {
				...
				if(p.buttonicon.toString().toUpperCase() == "NONE") {
				    ...
				    //PathSolutions: put the caption in a separate td so that when resizing to small width the text does not overlap the img
					tbd = $("<td class='ui-pg-button ui-corner-all'><div class='ui-pg-div'><span class='ui-icon "+p.buttonicon+"'></span></div></td><td>"+p.caption+"</td>");
				}
				
				if(p.id) {$(tbd).first("td").attr("id",p.id);}
				if(p.position=='first'){
